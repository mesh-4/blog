...
1. 當你的 width 或是 height 有指定的值，使用 unit base 很麻煩，但特地獨立出來一個 style 又感覺沒有必要。

```html
<!-- how to deal with 75px, 3.5vh, calc(100% - 34px) -->
<div class="w-4"></div>
```
...
不過當使用 UI 框架的時候，似乎每一個框架都很難在「全面客制」和「依賴(框架的)原生」之間達到平衡，大部分都是因為以下幾點：

1. 組件的 props 沒有提供全面的選項，提供的客制方法也會破壞檔案架構

面對一個 180 行但是只有 35 行是組件本體還是分成 small.jsx 和 small.style.js 但是 small.style.js 只有 7 行。

2. props 命名不明確，增加不必要的成本
...
當你不滿意 material ui 或 antd 那些一成不變的東西時，你可以選擇用 styled system，
不過你最好要有充足的時間，打造一個相較全面的元件庫的樣式設計、測試的流程的成本不比以上框架的各種客制成本低。

所以整理出來只要有個 UI 框架能做到以下幾點在我眼裡就代表「好」的框架：
...
沒錯，就是現在！ **@chakra-ui/react**正式發表了 1.0 版本，基於 styled-system 所建立起來的 UI 框架，
...
當你使用 framer-motion 的時候，組件可以透過各種 Transition 的組件去使用，
  ...
  return (
    <Box width="100%">
      <Fade
        in
        {/*
          you can treat it like motion.div
...
你可以依照這個[文檔](https://chakra-ui.com/docs/theming/customize-theme)作為標準來有規模地管理你所有的客制元件或是全局樣式。

你可以透過 cookie 或是 localStorage 去管理亮暗模式，在組件中使用 useColorMode 這個 hook 去提取和更換現在的亮暗模式。
